...
visualisePatient <- function(patientData,
  ...
  newPatientData <- dplyr::select(patientData,
                                  ...
                                  STATE_END_DATE)
  newPatientData <-
    dplyr::filter(newPatientData, SUBJECT_ID == patientId)
  newPatientData <-
  ...
  if (theme == 1) {
    ...
    for (index in 1:length(momentDifBoolean)) {
      if (!momentDifBoolean[index])
        ...
        momentStart[c(index + 1:n)] <-
          momentStart[c(index + 1:n)] - momentStart[index + 1]
        momentEnd[c(index + 1:n)] <-
          momentStart[c(index + 1:n)] + momentDuration[c(index +
                                                           1:n)]
  ...
  # Some Na's may occur
  momentStart <-
    as.vector(na.exclude(momentStart))
  momentEnd <-
    as.vector(na.exclude(momentEnd))
  
  # Color for visualisation
  n_states <-
    length(unique(newPatientData$STATE_LABEL))
  colors <- NULL
  # Due to RColorBrewer limitations we repeat if > 12 classes
  if (n_states > 12) {
    n <- 12
    colors_vec <-
      colors = RColorBrewer::brewer.pal(n = n, name = 'Paired')
    while (n < n_states) {
  ...
  newPatientData <-
    dplyr::filter(newPatientData, INCLUDE == TRUE)
  newPatientData <-
    merge(x = newPatientData,
          y = colorTable,
          by = "STATE_LABEL",
          all.x = TRUE)
  newPatientData <-
  ...
  blank_data <-
    data.frame(GROUP = sort(unique(groups)),
               y = 1,
               x = as.vector(
...
visualiseStateOverlap <- function(patientData, stateLabels) {
  ...
  colnames(patientDataCopy) <-
    c("SUBJECT_ID",
      ...
      "STATE_END_DATE")
  patientDataCopy <-
    dplyr::filter(patientDataCopy, STATE_LABEL != '0')
  ...
  for (patientId in unique(patientDataCopy$SUBJECT_ID)) {
    patientObservedData <-
      patientDataCopy[patientDataCopy$SUBJECT_ID == patientId,]
    patientObservedData <-
      dplyr::arrange(patientObservedData, STATE_START_DATE)
  ...
  plot <-
    pheatmap::pheatmap(matrix, cluster_rows = F, cluster_cols = F)
  return(plot)
}
