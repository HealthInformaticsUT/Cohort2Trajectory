...
UNION  select c.concept_id
  ...
  and ca.ancestor_concept_id in (45567179,319835,316139,45581779,45576879)
  and c.invalid_reason is null

) I
LEFT JOIN
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (45576863,319034)
...
(	
	SELECT
		person_id
		, DATEADD(day,-1 * 31, event_date)  as end_date
	FROM
	(
		SELECT
			person_id
		...
		(
			...
			SELECT
				person_id
				, DATEADD(day,31,end_date) as end_date
				, 1 AS event_type
				, NULL
			FROM #cohort_rows
		) RAWDATA
